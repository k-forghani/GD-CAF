...
class Precipitation_base(GMAN_base):
    ...
    def prepare_data(self):
        self.new_dataset = CustomPrecipitationDataset(
        ...
        self.new_dataset.count_data_rows()
        self.new_dataset.load_grid()
        self.new_dataset.normalize()

        num_train = len(self.new_dataset)  # Number of training samples (total - past and future look)
    ...
    def train_dataloader(self):
        return DataLoader(self.new_dataset, batch_size=self.hparams.batch_size, sampler=self.train_sampler)

    def val_dataloader(self):
        return DataLoader(self.new_dataset, batch_size=self.hparams.batch_size, sampler=self.valid_sampler)

def get_test_dataset(dataset_path, past_look, future_look, B=16, fast_dev_run=False, cell_path='', cell_cutoff=100):
    new_dataset = CustomPrecipitationDataset(
    ...
    new_dataset.count_data_rows()
    new_dataset.load_grid()
    new_dataset.normalize()

    data_loader = DataLoader(new_dataset, batch_size=B, shuffle=False)
